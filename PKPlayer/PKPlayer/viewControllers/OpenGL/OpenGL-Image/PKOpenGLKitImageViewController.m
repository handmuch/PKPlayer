 ...
 */
typedef struct {
    GLKVector3 positionCoord; // (X, Y, Z) OpenGL ES 坐标系
    GLKVector2 textureCoord; // (U, V) 纹理坐标系
} SenceVertex;

@interface PKOpenGLKitImageViewController ()<GLKViewDelegate>
...
@implementation PKOpenGLKitImageViewController
...
- (void)commitInit {
    ...
    // 初始化 GLKView
    CGRect frame = CGRectMake(0, 100, [UIApplication sharedApplication].keyWindow.frame.size.width, [UIApplication sharedApplication].keyWindow.frame.size.width * 1.47); // 为了 OpenGL 坐标系不被拉伸，这里设置为正方形
